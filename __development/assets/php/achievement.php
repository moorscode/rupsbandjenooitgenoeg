...
if ( intval( $_SESSION['user_id'] ) > 0 ) {
	...
	$query = $db->query( "SELECT c.`id`, c.`timestamp`, a.`title`, a.`description` FROM `achievements__Achievements` AS a INNER JOIN `achievements__Completed` as c ON a.`id`=c.`achievement_id` WHERE c.`player_id`=" . $_SESSION['user_id'] . " AND c.`shown`=0 ORDER BY `timestamp` ASC" );
	if ( $achievement = $db->assoc( $query ) ) {
		// fetch data

		echo "<h3>Prestatie: " . $achievement['title'] . "</h3><br />'" . $achievement['description'] . "'<br /><small>gehaald op " . strftime( "%e %B %Y om %l:%M %p", $achievement['timestamp'] ) . "</small>";

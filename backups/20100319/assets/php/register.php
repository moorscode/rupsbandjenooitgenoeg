...
if($_SERVER['REQUEST_METHOD'] == "POST") {
	...
	if(empty($username) || $username == '') {
		push_message('<h2>Registratie fout!</h2><br />Je bent vergeten een alias op te geven.<br />Onder deze naam zal jij bekend worden bij de andere spelers, kies dus een naam die goed bij je past!');
		die("{'saved':false}");
	}
	
	if(!is_valid_email($email)) {
		push_message('<h2>Registratie fout!</h2><br />Het ingevoerde e-mail adres is ongeldig.<br /><br />Gebruik een geldig e-mail adres om te registreren<br />(hierop ontvang je een verificatie code om het account te kunnen gebruiken).');
		die("{'saved':false}");
	}
	...
	if($user = $db->assoc($result)) {
		// email taken.
		push_message("<h2>Registratie fout!</h2><br />Het door jou ingevoerde e-mail adres is al in gebruik!<br />Als dit een e-mail adres is wat je zelf bezit, controleer dan je Spam/Junk mappen.");
		die("{'saved':false}");
	}
	...
	if($test = $db->assoc($result)) {
		push_message("<h2>Registratie fout!</h2><br />Deze alias is al in gebruik. Kies een andere (of pas deze aan) en probeer opnieuw!");
		die("{'saved':false}");
	}
	
	if(empty($password) || $password == '') {
		push_message("<h2>Registratie fout!</h2><br />Je moet een wachtwoord opgeven!");
		die("{'saved':false}");
	}
	
	if($password2 != $password) {
		push_message("<h2>Registratie fout!</h2><br />De opgegeven wachtwoorden komen niet overeen, typ twee keer uw wachtwoord zodat u zeker weet dat deze klopt!");
		die("{'saved':false}");
	}
	...
	$time = time();
	$db->query("INSERT INTO `global__Players` (`email`, `password`, `username`, `verify`, `invite_code`, `timestamp`) VALUES ('$email', '$password', '$username', '$verify', MD5('$time'), UNIX_TIMESTAMP())");
...
<tr>
	...
	<td><input type="email" name="email" value="" maxlength="255" autocomplete="no" size="30" /></td>
</tr><tr>
...
Tevens verstrekken wij deze adressen nooit aan derden.</small>
</center>
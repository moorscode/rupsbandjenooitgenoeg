...
while($ui = $db->assoc($ui_query)) {
	...
	array_push($user_input[$ui['phrase']], array('id'=>$ui['id'], 'pid'=>$ui['player_id'], 'username'=>$ui['username'], 'text'=>$ui['text'], 'timestamp'=>$ui['timestamp']));
}
$db->free($ui_query);
...
<div id="dynamicForm">
	...
	<div id="form"></div>
	<div class="button"><input type="submit" id="dynamicFormSubmit" value=" opslaan " /> <input type="button" id="dynamicFormCancel" value=" annuleer " /></div>
	</form>
</div>
...
<div id="adminBody">
	...
	<table width="580">
	<tr><td><input type="checkbox" value="1" name="access" id="access"<?php echo $access ?>/><label for="access">access granted</label></td><td align="right"><input type="submit" value=" apply " /></td></tr>
	</table>
	</form>
	</fieldset>
	...
	<table width="580">
	<tr>
		<td width="16"><img id="chatStatus" src="../assets/html/images/<?= (isset($chat_id)?"online":"offline") ?>.gif" width="12" height="11" /></td><td>Chat Server</td><td align="right"><input type="button" name="chatControl" value=" <?= (isset($chat_id)?"kill":"start") ?> " onclick="location.href='./?do=<?= (isset($chat_id)?"kill":"start") ?>&what=chat';" /></td>
	</tr>
	<tr>
		<td width="16"><img id="chatStatus" src="../assets/html/images/<?= (isset($game_id)?"online":"offline") ?>.gif" width="12" height="11" /></td><td>Game Server</td><td align="right"><input type="button" name="chatControl" value=" <?= (isset($game_id)?"kill":"start") ?> " onclick="location.href='./?do=<?= (isset($game_id)?"kill":"start") ?>&what=game';" /></td>
	</tr>
	</table>
	</fieldset>
...
?>
	...
	Find user: <input type="text" name="find" id="find" onchange="updateUserList(this.value);"  onkeyup="updateUserList(this.value);" size="40" autocomplete="no" value="" /><br />
...
?>
	...
	<table width="580" bgcolor="#efefef" cellpadding="5" cellspacing="0" style="border: 1px dotted black; border-bottom: 0px;">
<?php

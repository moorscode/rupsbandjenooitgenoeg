   ...
   function on_input($client_index) {
		...
		switch($what) {
			...
			case 'create': // create a new game
				...
				array_push($this->games, array("id"=>$game_id, "name"=>$name, "players"=>array($user_id), "max_players"=>$max_players, "password"=>$password, "queue_time"=>time()));
	...
	function send_all_gamelist() {
		...
		for($c = 0; $c < count($this->client); $c++) {
			if(isset($this->client[$c]['game_id'])) {
				...
				if($game_index > -1 && $this->games[$game_index]['active']) continue;
			}
	...
	function send_players($game_id, $xml, $__exclude, $type) {
		...
		for($p = 0; $p < count($game["players"]); $p++) {
			...
			if($__exclude > -1 && $__exclude == $user_id) continue;

...
foreach($categories as $category) {
	$menu .= ($menu == "") ? "" : " | ";
	$menu .= "<a href=\"#".$category['name']."\">".$category['name']."</a>";
}
...
if (AC_FL_RunContent == 0 || DetectFlashVer == 0) {
	...
	if(hasRightVersion) { 
		AC_FL_RunContent(
			...
			'FlashVars', 'library_path=../assets/charts/charts_library&xml_source=../assets/charts/data/stats.xml.php?type=<?= $chart['id'] ?>', 
			'id', '<?= $chart['id'] ?>',
			'name', '<?= $chart['id'] ?>',
			'menu', 'true',

...
class GameServer extends SocketServer {
		...
		switch ( $what ) {
			...
			case 'start':
				...
				if ( $client['user_id'] == $this->games[ $game_index ]["players"][0] ) {
					...
					// send to all in game...
					$this->send_queued_players( $game_id, "<w>startgame</w><st>" . time() . "</st>" );
			...
			case 'level':
				...
				$this->games[ $game_index ]['start_positions'] = $positions - 1;
			...
			case 'startpos':
				...
				$this->send_game_players( $game_id, "<w>startpos</w><i>$user_id</i><pos>$start_position</pos>" );
				break;
	...
	function send_players( $game_id, $xml, $__exclude, $type ) {
		...
		for ( $p = 0; $p < count( $game["players"] ); $p++ ) {
			...
			if ( $__exclude > -1 && $__exclude == $client_index ) {
				continue;
			}
